...
describe("RoomClient", () => {
  it("queues a create until the socket opens, then sends it", () => {
    ...
    sock.open();
    expect(sock.last()).toEqual({ t: "create", deck: DECK });
  });
  ...
  it("delivers the start payload with role and both decks", () => {
    ...
    sock.open();
    sock.emit({ t: "start", role: "guest", hostDeck: DECK, guestDeck: DECK });
    expect(onStart).toHaveBeenCalledWith({ role: "guest", hostDeck: DECK, guestDeck: DECK });
  });
