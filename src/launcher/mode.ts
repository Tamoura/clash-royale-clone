 ...
 *
 * First-time visitors have no stored choice ("unset") until they tap an edition.
 */
import { ARENA_THEME_KEY, type ArenaTheme, type StoredEdition } from "../render3d/theme";

export type GameMode = "clash" | "islamic";
/** No preferred default — the lobby forces an equal pick. */
export const DEFAULT_MODE: GameMode | null = null;
...
export function themeToMode(theme: string | null): GameMode | null {
  if (theme === "normal") return "clash";
  if (theme === "arabic") return "islamic";
  return null;
}

/** Read the stored edition; null when the player has not chosen yet. */
export function loadMode(storage: ModeStorage): GameMode | null {
...
export function isStoredEdition(value: string | null): value is StoredEdition {
  return value === "normal" || value === "arabic" || value === "unset";
}
